...
import { ArrowRight, Sparkles, Eye, EyeOff, Target, FileUp, TrendingUp } from "lucide-react";
import { useAuth } from "../../context/AuthContext";
import { Button } from "../ui";

const TAGLINES = [
  "Free forever for individuals — no credit card required.",
  "Every recommendation is backed by real job postings.",
  "Auto-parse your skills straight from your resume.",
  "See how skill demand shifts over time, not just a snapshot.",
];

const FEATURES = [
  { icon: Target, label: "Evidence-backed" },
  { icon: FileUp, label: "Resume auto-parse" },
  ...
  return (
    <div className="grid min-h-screen grid-cols-1 bg-cream lg:grid-cols-2">
      <div className="relative hidden flex-col justify-between overflow-hidden bg-lime p-10 lg:flex">
        <FloatingCircle size={140} top="-40px" left="-40px" delay={0} color="var(--periwinkle)" />
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="mt-6 flex flex-col gap-4">
              <label className="flex flex-col gap-1.5">
                <span className="text-xs font-semibold text-forest/70">Email</span>
                <div className="relative">
                  <input
                    type="email"
                    autoComplete="email"
                    ...
                    className="h-11 w-full rounded-xl border border-forest/15 bg-white px-4 pr-9 text-sm text-forest outline-none transition-colors placeholder:text-forest/35 focus:border-forest/40"
                  />
                  {email.length > 0 && emailLooksValid && (
                    <span className="absolute right-3 top-1/2 -translate-y-1/2 text-lime-600">
                      <svg viewBox="0 0 20 20" fill="currentColor" className="h-4 w-4" style={{ color: "var(--lime)" }}>
                        <path
                          fillRule="evenodd"
