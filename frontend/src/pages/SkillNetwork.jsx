...
function TopPairs({ edges }) {
  ...
  return (
    <ul className="flex flex-col gap-2">
      {top.map((edge) => (
        <li key={`${edge.source}-${edge.target}`} className="flex items-center justify-between rounded-xl border border-forest/10 bg-white px-4 py-2.5 text-sm">
    ...
    <section className="mx-auto max-w-6xl px-6 py-12 sm:py-16">
      ...
      {state.status === "ready" && (
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1.2fr_1fr]">
          ...
          <Card eyebrow="Strongest connections" title="Skills that travel together most">
